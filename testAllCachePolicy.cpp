...
void testHotDataAccess() {
    ...
    const int COLD_KEYS = 6000;      // 冷数据数量
    
    KunCache::KLruCache<int, std::string> lru(CAPACITY);
    KunCache::KLfuCache<int, std::string> lfu(CAPACITY);//这里的maxaverage是默认值
    KunCache::KArcCache<int, std::string> arc(CAPACITY/2);
    // 为LRU-K设置合适的参数：
    // - 主缓存容量与其他算法相同
    // - 历史记录容量设为可能访问的所有键数量
    // - k=2表示数据被访问2次后才会进入缓存，适合区分热点和冷数据
    KunCache::KLruKCache<int, std::string> lruk(CAPACITY, HOT_KEYS + COLD_KEYS, 2);
    KunCache::KLfuCache<int, std::string> lfuAging(CAPACITY, 20000);//这里的maxaverage被修改为20000

    std::random_device rd;
    std::mt19937 gen(rd());
    
    // 基类指针指向派生类对象，添加LFU-Aging
    std::array<KunCache::KICachePolicy<int, std::string>*, 5> caches = {&lru, &lfu, &arc, &lruk, &lfuAging};
...
void testLoopPattern() {
    ...
    const int OPERATIONS = 150000;    // 总操作次数
    
    KunCache::KLruCache<int, std::string> lru(CAPACITY);
    KunCache::KLfuCache<int, std::string> lfu(CAPACITY);
    KunCache::KArcCache<int, std::string> arc(CAPACITY/2);
    // 为LRU-K设置合适的参数：
    // - 历史记录容量设为总循环大小的两倍，覆盖范围内和范围外的数据
    // - k=2，对于循环访问，这是一个合理的阈值
    KunCache::KLruKCache<int, std::string> lruk(CAPACITY, LOOP_SIZE * 2, 2);
    KunCache::KLfuCache<int, std::string> lfuAging(CAPACITY, 3000);
...
void testWorkloadShift() {
    ...
    KunCache::KLfuCache<int, std::string> lfu(CAPACITY);
    KunCache::KArcCache<int, std::string> arc(CAPACITY/2);
    KunCache::KLruKCache<int, std::string> lruk(CAPACITY, 500, 2);
    KunCache::KLfuCache<int, std::string> lfuAging(CAPACITY, 10000);
